...
def reg_missing_3d(data, mis_interval=2, mode='inline'):
    '''
    Regular missing traces for 3D seismic cube.
    ...
    INPUT:
    data: 3D seismic cube (dimensions: inline, xline, time)
    mis_interval: Interval at which traces will be set to zero (default: 2)
    mode: Mode of trace removal ('inline' or 'crossline', default: 'inline')
    
    OUTPUT:
    3D seismic cube with missing traces at the specified intervals.
    '''
    # Convert input data to numpy array (if not already a numpy array)
    seis_data = np.array(data)
    
    # Check if mis_interval is a positive integer
    if not isinstance(mis_interval, int) or mis_interval <= 0:
        raise ValueError("mis_interval should be a positive integer.")
    
    # Apply missing traces based on mode
    if mode == 'inline':
        # Set every 'mis_interval' -th trace along the time axis to 0
        seis_data[:, :, ::mis_interval] = 0

    elif mode == 'crossline':
        # Set every 'mis_interval' -th trace along the xline axis to 0
        seis_data[:, ::mis_interval, :] = 0

    else: 
        raise ValueError('Invalid mode. Please choose "inline" or "crossline".')
